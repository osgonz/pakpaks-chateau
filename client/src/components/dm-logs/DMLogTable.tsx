...
const DMLogTable = (props: DMLogTableProps) => {
  ...
  const { order, sort, setOrderSort, page, setPage, rows, setRows } =
    useTableSearchParams(["timestamp", "title", "serviceHours"]);
  ...
  const visibleRows = useMemo(
    () =>
      logs
        .slice()
        .sort(getSortComparator(order, sort as keyof DMLog))
  ...
  const deleteLog = async () => {
    ...
    props.handleRemoveLogByIndex(
      logs!.findIndex((log) => log.id === logToBeDeleted?.id)
    );
    handleDeleteClose();
  };
        ...
        <Table sx={{ minWidth: 650 }} size="medium" aria-label="DM Logs table">
          ...
          <TableBody>
            {visibleRows.map((log) => (
              <ExpandableTableRow
                id={log.id}
                expandableContent={log.description ?? ""}
              >
                <TableCell>
