...
import { CharacterLog } from "../data/Types";
...
export function useCharacterLogsByCharacter(characterId: string) {
    const [characterLogs, setCharacterLogs] = useState<CharacterLog[] | undefined>();
    ...
    const loadCharacterLogsByCharacter = async(characterId: string) => {
        return axios.get(`/api/characters/${characterId}/character-logs`).then((res) => res.data) as Promise<CharacterLog[]>;
    };
